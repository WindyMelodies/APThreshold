...
from utils.particle_swarm_optimizer import PSO
from utils.threshold_equation import FormOne, solve_threshold_model_euler, FormTwo, FormThree
import cma
from utils.tools import get_voltage_from_data, get_timestamp_from_data
...
class ConfigureEquationParametersWindow(QWidget, Ui_ConfigureEquationParametersWindow):
    ...
    def __init__(self, equation_index, equation_option_combobox, main_window):
        ...
        self.params_res = {}
        self.equation_pack = {'1': FormOne, '2': FormTwo, '3': FormThree}
    ...
    def fit_equation_parameters(self):
        ...
        try:
            ...
            if optimization_algorithm_option == 1:
                x0 = self.get_initial_value()
                sigma = self.doubleSpinBox_cmaes_sigma.value()
                pop_size = self.spinBox_cmaes_popsize.value()
                max_iter = self.spinBox_cmaes_maxiter.value()
                tol_fun = self.doubleSpinBox_cmaes_tolfun.value()

                es = cma.CMAEvolutionStrategy(x0=x0, sigma0=sigma, inopts={'bounds': bounds},
                                              options={'maxiter': f'{max_iter}', 'popsize': f'{pop_size}',
                                                       'tolfun': f'{tol_fun}'})  # 带边界约束的CMA-ES优化
                es.optimize(objective_function, args=[kwargs])
                es.result_pretty()
                result = es.result
                best_solution = result[0]  # 最优解
            elif optimization_algorithm_option == 2:
                max_iter = self.spinBox_max_iter.value()
                pop_size = self.spinBox_pop_size.value()
                w_ini = self.doubleSpinBox_w_ini.value()
                w_fin = self.doubleSpinBox_w_end.value()
                c1 = self.doubleSpinBox_c1.value()
                c2 = self.doubleSpinBox_c2.value()
                ftol = self.doubleSpinBox_ftol.value()
                ftol_iter = self.spinBox_ftol_iter.value()
                pso = PSO(dim=np.size(x0), lb=bounds[0], ub=bounds[1], c1=c1, c2=c2, w_fin=w_fin, w_ini=w_ini,
                          pop_size=pop_size)
                _, best_solution = pso.optimize(objective_function, kwargs=kwargs, ftol=ftol, ftol_iter=ftol_iter,
                                                max_iter=max_iter)
    ...
    def result_visualization(self, estimated_spike_threshold, **kwargs):
        ...
        y_max = max(voltage)
        y_min = min(voltage)
        for i in time_threshold_estimation:
        ...
        save = False
        if save:
            save_dict = {'time_threshold_predict': time_threshold_predict,
                         "time_threshold_estimation": time_threshold_estimation, "voltage": voltage,
                         "timestamp": timestamp, "estimated_spike_threshold": estimated_spike_threshold,
                         "threshold_predict": threshold_predict}
            with open("fitting_res_0830.pkl", "wb") as f:
        ...
        ax_4.scatter(estimated_spike_threshold,threshold_predict)
        ax_4.plot(estimated_spike_threshold,estimated_spike_threshold)
        ax_4.set_xlabel('Estimated Vth (mV)')
        ax_4.set_ylabel('Predicted Vth (mV)')
        self.main_window.names['Canvas_Vth'].draw()

    def update_equation_dynamics_param_info(self):

        self.param_info_equation_dynamics = {
            'tau_theta': {"bounds": [], "final_value": 0.0},
            'alpha': {"bounds": [], "final_value": 0.0},
            'ka': {"bounds": [], "final_value": 0.0},
            'ki': {"bounds": [], "final_value": 0.0},
            'Vi': {"bounds": [], "final_value": 0.0},
            'VT': {"bounds": [], "final_value": 0.0}
        }

        self.param_info_equation_dynamics['tau_theta'][
            'final_value'] = self.doubleSpinBox_tao_theta_dynamics_fixed.value()

        self.param_info_equation_dynamics['alpha'][
            'final_value'] = self.doubleSpinBox_alpha_dynamics_fixed.value()

        self.param_info_equation_dynamics['ka'][
            'final_value'] = self.doubleSpinBox_ka_dynamics_fixed.value()

        self.param_info_equation_dynamics['ki'][
            'final_value'] = self.doubleSpinBox_ki_dynamics_fixed.value()

        self.param_info_equation_dynamics['Vi'][
            'final_value'] = self.doubleSpinBox_Vi_dynamics_fixed.value()

        self.param_info_equation_dynamics['VT'][
            'final_value'] = self.doubleSpinBox_VT_dynamics_fixed.value()
    ...
    def get_initial_value(self):
        equation_index = self.stackedWidget_equation_parameters.currentIndex() + 1

        initial_value = []
        if equation_index == 1:
            initial_value.append(self.doubleSpinBox_tao_theta_dynamics_initial.value())
            initial_value.append(self.doubleSpinBox_alpha_dynamics_initial.value())
            initial_value.append(self.doubleSpinBox_ka_dynamics_initial.value())
            initial_value.append(self.doubleSpinBox_ki_dynamics_initial.value())
            initial_value.append(self.doubleSpinBox_Vi_dynamics_initial.value())
            initial_value.append(self.doubleSpinBox_VT_dynamics_initial.value())
        elif equation_index == 2:
            initial_value.append(self.doubleSpinBox_tao_theta_origin_initial.value())
            initial_value.append(self.doubleSpinBox_ka_origin_initial.value())
            initial_value.append(self.doubleSpinBox_ki_origin_initial.value())
            initial_value.append(self.doubleSpinBox_Vi_origin_initial.value())
            initial_value.append(self.doubleSpinBox_VT_origin_initial.value())
        elif equation_index == 3:
            initial_value.append(self.doubleSpinBox_tao_theta_rectifier_initial.value())
            initial_value.append(self.doubleSpinBox_alpha_rectifier_initial.value())
            initial_value.append(self.doubleSpinBox_Vi_rectifier_initial.value())
            initial_value.append(self.doubleSpinBox_VT_rectifier_initial.value())
    ...
    def update_equation_origin_param_info(self):
        ...
        self.param_info_equation_origin['ka'][
            'final_value'] = self.doubleSpinBox_ka_origin_fixed.value()

        self.param_info_equation_origin['ki'][
            'final_value'] = self.doubleSpinBox_ki_origin_fixed.value()

        self.param_info_equation_origin['Vi'][
            'final_value'] = self.doubleSpinBox_Vi_origin_fixed.value()

        self.param_info_equation_origin['VT'][
            'final_value'] = self.doubleSpinBox_VT_origin_fixed.value()
    ...
    def update_equation_rectifier_param_info(self):
        ...
        self.param_info_equation_rectifier['tau_theta'][
            'final_value'] = self.doubleSpinBox_tao_theta_rectifier_fixed.value()

        self.param_info_equation_rectifier['alpha'][
            'final_value'] = self.doubleSpinBox_alpha_rectifier_fixed.value()

        self.param_info_equation_rectifier['Vi'][
            'final_value'] = self.doubleSpinBox_Vi_rectifier_fixed.value()

        self.param_info_equation_rectifier['VT'][
            'final_value'] = self.doubleSpinBox_VT_rectifier_fixed.value()
    ...
    def ok_clicked(self):
        ...
        # 2. 最优化参数部分数据进行传递
        if (self.stackedWidget_equation_parameters.currentIndex() + 1) == 1:
            self.get_equation_param_value_form_one()
        elif (self.stackedWidget_equation_parameters.currentIndex() + 1) == 2:
            self.get_equation_param_value_form_two()
        elif (self.stackedWidget_equation_parameters.currentIndex() + 1) == 3:
            self.get_equation_param_value_form_three()
        self.close()

    def get_equation_param_value_form_one(self):
        self.params_res = {}
    ...
    def get_equation_param_value_form_two(self):
        self.params_res = {}
        self.params_res["tau_theta"] = self.doubleSpinBox_tao_theta_origin_fixed.value()
        self.params_res['ka'] = self.doubleSpinBox_ka_origin_fixed.value()
        self.params_res['ki'] = self.doubleSpinBox_ki_origin_fixed.value()
        self.params_res['Vi'] = self.doubleSpinBox_Vi_origin_fixed.value()
        self.params_res['VT'] = self.doubleSpinBox_VT_origin_fixed.value()

    def get_equation_param_value_form_three(self):
        self.params_res = {}
        self.params_res["tau_theta"] = self.doubleSpinBox_tao_theta_rectifier_fixed.value()
        self.params_res["alpha"] = self.doubleSpinBox_alpha_rectifier_fixed.value()
        self.params_res['Vi'] = self.doubleSpinBox_Vi_rectifier_fixed.value()
        self.params_res['VT'] = self.doubleSpinBox_VT_rectifier_fixed.value()
